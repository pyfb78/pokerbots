...
struct Bot {
  ...
  */
  void handleRoundOver(GameInfoPtr gameState, TerminalStatePtr terminalState,
    ...
    // cards or "" if not revealed bool myBountyHit =
    // terminalState->bounty_hits[active];  // true if your bounty hit this
    // round bool oppBountyHit = terminalState->bounty_hits[1-active];  // true
    // if your opponent's bounty hit this round
  }
  ...
  */
  Action getAction(GameInfoPtr gameState, RoundStatePtr roundState,
    ...
    // flop, turn, or river respectively auto myCards =
    // roundState->hands[active];  // your cards auto boardCards =
    // roundState->deck;  // the board cards int myPip =
    // roundState->pips[active];  // the number of chips you have contributed to
    // the pot this round of betting int oppPip = roundState->pips[1-active]; //
    // the number of chips your opponent has contributed to the pot this round
    // of betting int myStack = roundState->stacks[active];  // the number of
    // chips you have remaining int oppStack = roundState->stacks[1-active];  //
    // the number of chips your opponent has remaining int continueCost = oppPip
    // - myPip;  // the number of chips needed to stay in the pot int
    // myContribution = STARTING_STACK - myStack;  // the number of chips you
    // have contributed to the pot int oppContribution = STARTING_STACK -
    // oppStack;  // the number of chips your opponent has contributed to the
    // pot char myBounty = roundState->bounty[active];  // your current bounty
    // rank if (legalActions.find(Action::Type::RAISE) != legalActions.end()) {
    //   auto raiseBounds = roundState->raiseBounds();  // the smallest and
    //   largest numbers of chips for a legal bet/raise int minCost =
    //   raiseBounds[0] - myPip;  // the cost of a minimum bet/raise int maxCost
    //   = raiseBounds[1] - myPip;  // the cost of a maximum bet/raise
    // }
